...
ACTIVE_STRATEGIES = {
    ...
    'v11_var13',
    # V11.5 (VAR-13) shadow: venue 別 2着/3着 specialist + QMC v4 — 2026-06-04 並走開始
    'v11_5_var13',
}
    ...
    def calculate_all_strategies(self, probs_1st, probs_2nd, probs_3rd,
        ...
        for strategy_name, strategy_config in self.config['strategies'].items():
            ...
            if bet_mode == 'nirentan':
                ...
                # モンテカルロ確率（mc_version で v1/QMC/v3/v4 分岐）
                mc_ver = strategy_config.get('mc_version', 1)
                if mc_ver >= 4:
                    # QMC v4: V11.5 specialist 群 (1着/2着/3着) + 既存 std 調整 + Plackett-Luce
                    cache_key = f'qmc_v4_{strategy_name}'
                    if cache_key not in _mc_cache:
                        from src.monte_carlo import qmc_sanrentan_v4
                        _mc_cache[cache_key] = qmc_sanrentan_v4(
                            probs_1st, probs_2nd, probs_3rd,
                        ...
                        logger.info(
                            f"QMC v4確率生成: {len(_mc_cache[cache_key])}通り "
