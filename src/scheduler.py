...
# 締切前リードタイム（分）
# 3〜5分前に処理。3件購入で約60秒、+30秒境界(画面が次レースに切替わる時刻)を
# 確実に超えないため余裕を持たせる。
# 2026-05-05 1.5→3, 3→5 に拡張 (P6 で締切超過による誤レース購入が頻発したため)
# EV影響: 1.5分前 vs 3分前のオッズ変動はほぼ同等 (実測: avg %差 12.5% vs 14.8%)
LEAD_TIME_MIN = 3    # 最小リードタイム（これ未満はスキップ）
